...
- **`OLE` (Output Limit Exceeded)** — the program printed more than the allowed [output limit](#output-limit); the Runner also raises this if a program in an interactive setup causes its parent to overflow.
...
- **`MLE` (Memory Limit Exceeded)** — blew past the [memory limit](#memory-limit).
...
## Contests, courses, and their shared plumbing

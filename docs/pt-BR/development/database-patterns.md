...
```
Em caso de acerto, ele retorna o valor armazenado em cache e o fechamento nunca é executado; em caso de falha, ele executa o fechamento, armazena o resultado na chave com o TTL fornecido e o retorna. Sob o capô `set` / `get` / `delete` cada curto-circuito quando o cache é desabilitado (`isEnabled()` falso) - `get` retorna `null`, `set` e `delete` retorna `false` - então uma caixa sem back-end de cache degrada para atingir o MySQL todas as vezes em vez de travar, e cada acerto/erro/armazenamento é registrado Monolog sob o nome de canal `cache` para quando você precisar ver por que algo está ou não sendo servido no cache.

...
## O caminho completo, de ponta a ponta {#the-whole-path-end-to-end}
...
```
## Estágio 1 — CI deve passar antes da mesclagem {#stage-1-ci-must-pass-before-merge}

Antes que um PR possa ser mesclado com `main`, ele deve estar verde no fluxo de trabalho [`CI`](https://github.com/omegaup/omegaup/blob/main/.github/workflows/ci.yml), que é executado em cada `pull_request` e em cada `push` a `main` (e também é exposto como um `workflow_call` reutilizável). Um grupo `concurrency` digitado no número PR cancela qualquer execução em andamento quando você envia um novo commit - `cancel-in-progress: true` - para que você nunca queime executores testando duas vezes uma revisão obsoleta.
...
### Cypress é executado em quatro fragmentos {#cypress-runs-in-four-shards}
...
## Estágio 2 — mesclar com `main` cria e implanta sandbox {#stage-2-merge-to-main-builds-and-deploys-sandbox}
...
### As cinco imagens que construímos {#the-five-images-we-build}
...
### Cada imagem é marcada duas vezes, enviada para dois registros {#every-image-is-tagged-twice-pushed-to-two-registries}
...
### GitOps: a implantação é um compromisso com `omegaup/prod` {#gitops-the-deploy-is-a-commit-to-omegaupprod}
...
## Promoção programada para produção {#scheduled-promotion-to-production}

A produção não é implantada por um clique humano em um botão. Dois fluxos de trabalho agendados promovem `main` em `release` e, uma vez que `release` é movido, o [Estágio 2](#stage-2-merge-to-main-builds-and-deploys-sandbox) faz o resto automaticamente:

- [`daily-release.yml`](https://github.com/omegaup/omegaup/blob/main/.github/workflows/daily-release.yml) — `cron: '0 3 * * 0'`, ou seja, **Domingos às 03:00 UTC (20:00 PT)**. Apesar do nome, atualmente funciona semanalmente, não diariamente; o arquivo ainda carrega um `TODO(#1624): Make this daily once we have better coverage of the frontend`, o que é uma observação sincera de que ainda não confiamos no pacote automatizado o suficiente para promovê-lo todos os dias.
...
## Lançamento de serviços de back-end em sua própria cadência {#backend-services-release-on-their-own-cadence}
...
- **[Configuração do Docker](docker-setup.md)** — o `docker-compose.yml` local empilha essas imagens espelhadas
